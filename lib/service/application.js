...
function Application(name, cb){
  ...
  self.setRuntimeStatus = function ( status ) {
    if ( self.hasOwnProperty( 'appId' ) && !_.isUndefined( self.appId ) && !_.isUndefined( status ) ) {
      ...
      fb.app.updateStatus(Matrix.deviceToken, self.appId, status, {
        error: function (err) {
          debug('update status app failed');
        },
        finished: function () {
          debug('update status app success');
        },
        start: function(){
          debug('update status app start');
        },
        progress: function () {
          debug('update status app in progress');
        }
      });
    } else {
